...
def parse_edf_eyelink(edf_file_path, msg_keywords,session_folder_path,force_best_eye,keep_ascii):
    """
    ...
    for iLine in range(nLines):    
        ...
        if '!MODE RECORD' in fileTxt0[iLine]:
            ...
            rate_recorded = float(fileTxt0[iLine-1].split('RATE')[-1].split('TRACKING')[0])
    ...
    dict_events = {'fix': dfFix, 'sacc': dfSacc, 'blink': dfBlink}
    if not keep_ascii:
...
def process_edf_file(edf_file_path, msg_keywords, session_folder_path,force_best_eye,keep_ascii):
    parse_edf_eyelink(edf_file_path, msg_keywords, session_folder_path,force_best_eye,keep_ascii)

def process_session(bids_dataset_folder, subject, session, msg_keywords, derivatives_folder,num_threads,force_best_eye,keep_ascii):
    ...
    with ThreadPoolExecutor(max_workers=num_threads) as executor:
        ...
        for file in edf_files:
            ...
            futures.append(executor.submit(process_edf_file, edf_file_path, msg_keywords, session_folder_path,force_best_eye,keep_ascii))
        for future in futures:
            future.result() # This will raise exceptions if any occurred during processing

def process_subject(bids_dataset_folder, subject, msg_keywords, derivatives_folder,num_threads,force_best_eye,keep_ascii):
    ...
    for session in sessions_folders:
        process_session(bids_dataset_folder, subject, session, msg_keywords, derivatives_folder,num_threads,force_best_eye,keep_ascii)

def compute_derivatives_for_dataset(bids_dataset_folder, msg_keywords,num_processes=None,num_threads=None,force_best_eye=True,keep_ascii=True):
    '''
    ...
    with ProcessPoolExecutor(max_workers=num_processes) as executor:
        ...
        for subject in bids_folders:
            futures.append(executor.submit(process_subject, bids_dataset_folder, subject, msg_keywords, derivatives_folder,num_threads,force_best_eye,keep_ascii))
