...
def get_ordered_trials_from_psycopy_logs(dataset_folder_path:str,trial_beginning_delimiter:str,trial_end_delimiter:str):
    ...
    source_root = dataset_folder_path / "sourcedata"
    if not source_root.exists():
        source_root = dataset_folder_path
    subjects = [subject for subject in source_root.iterdir() if subject.is_dir() and subject.name.startswith("sub-")]
