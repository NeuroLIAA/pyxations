...
Despite the central role of eye movements in vision research, there is currently no standardized pipeline or framework for organizing, parsing, and analyzing eye-tracking data equivalent to the Brain Imaging Data Structure (BIDS) [@gorgolewski2016bids]. The heterogeneity of experimental paradigms, eye-tracking devices, and event-detection algorithms, combined with the variability introduced by calibration procedures and file formats, makes it difficult to move from raw recordings to reproducible and comparable results. This lack of standardization also complicates cross-study and cross-laboratory comparisons, as even similar experiments often yield data in incompatible formats or rely on different preprocessing pipelines.
